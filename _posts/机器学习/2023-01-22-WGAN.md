...
$$L(G,D)=\int p_\text{data}(x)\log D(x)+p_g(x)\log(1-D(x))\text{d}x$$

$$D^*=\arg\max_{D(x)} L(G,D)=\frac{p_\text{data}(x)}{p_\text{data}(x)+p_g(x)}$$

$$\max_DL(G,D)=\mathbb{E}_{x\sim p_{data}(x)}[\log D^*]+\mathbb{E}_{x\sim p_g(x)}[\log(1-D^*)]$$

&emsp;&emsp;Given the global optimality of $p_g=p_\text{data}$, the optimal $D^*(x)$ becomes $1/2$ and the minimum of the $L(G,D)$ is $-2\log2$. Rethinking the distance between $p_\text{data}$ and $p_g$, we can get

$$JS(p_\text{data}\Vert p_g)=\log2+\frac{1}{2}L(G,D)$$

$$L(G,D)=2JS(p_\text{data}\Vert p_g)-2\log2$$

&emsp;&emsp;In essence, the loss function of GAN quantifies the distance between the real data distribution $p_\text{data}$ and the generative data distribution $p_g$. According to the JS-divergence, the lower bound of $L(G,D)$ is also $-2\log2$.
...
&emsp;&emsp;The other possible cause is that $p_\text{data}$ and $p_g$ rest in low dimensional manifolds, where they are almost disjointed. In this case the optimal discriminator will be perfect and its gradient will be zero almost everywhere. When the discriminator is perfect, the generator will hardly update due to vanishing gradients.
...
&emsp;&emsp;**(1) Adding noises.** Vanishing gradients always occurs in that $p_\text{data}$ and $p_g$ are disjoint. We can add continuous noise to the inputs of the discriminator, therefore smoothening the distribution of the probability mass.

&emsp;&emsp;**(2) Softer metrics of distribution distance.** When $p_\text{data}$ and $p_g$ are disjoint, the JS-divergence can not provide a meaningful value. Wasserstein metric is introduced to replace JS-divergence due to its better performance.
...
$$L(p_\text{data},p_g)=W(p_\text{data},p_g)=\max_{\omega}\mathbb{E}_{x\sim p_\text{data}}[f_\omega(x)]-\mathbb{E}_{z\sim p(z)}[f_\omega(g_\theta(z))]$$
...
&emsp;&emsp;Empirically, the WGAN recommended taking RMSProp or SGD as the optimizer rather than a momentum-based optimizer such as Adam. [Gulrajani](https://arxiv.org/abs/1704.00028) proposed an alternative way to enforce the Lipschitz constraint via gradient penalty as follows. $p(x)$ is sampled uniformly along straight lines between pairs of points sampled from $p_\text{data}$ and $p_g$. See the original paper for more details.

$$L=\underbrace{-\mathbb{E}_{x\sim p_\text{data}}[D(x)]+\mathbb{E}_{x\sim p_g(x)}[D(x)]}_{\text{Original critic loss}}+\lambda\mathbb{E}_{x\sim p(x)}[(\Vert\nabla_xD(x)\Vert_2-1)^2]$$
