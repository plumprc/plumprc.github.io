...
&emsp;&emsp;综上所述，同一个线性变换，可以用不同的基来显式表示，事实上这些基之间的关系为同构（isomorphism）。

## 特征向量与动力系统
// TODO: 先讲特征向量，到对角化，到线性变换视角下基的变换，最后再求解这个动力系统。

&emsp;&emsp;在欧氏空间的视角下，矩阵变换可视作对空间内所有的点进行拉伸和旋转
...
&emsp;&emsp;求矩阵的幂一直是比较复杂的问题，因为我们知道单个矩阵事实上同时进行了拉伸和旋转两类操作，其复合后的变换的复杂程度几乎无法用图像表征。假设存在这样的向量，矩阵对其只进行拉伸变换，我们就可以用拉伸系数的幂来表示矩阵的复合变换。我们称满足 $M\pmb{x}=\lambda\pmb{x}$ 的非零向量为特征向量（eigen vector），拉伸系数 $\lambda$ 为特征值（eigen value）；关于如何求解特征向量及特征值，在此不作赘述。

TODO: 求解这个动力系统，补充线性变换视角下基的变换